...
$URL = "https://api.telegram.org/bot" . Yii::$app->params['telegramToken'];
//$VK_ACCESS_TOKEN = "eb72b0b6eb72b0b6eb72b0b6cfeb2e57d8eeb72eb72b0b6b23c5b75d5081ce705e04377";
...
function sendMessage($URL, $mysqli, $chat_id, $message, $needStat, $keyboard, $oldMessage, $lang)//отправить сообщение
{
    if (isset($oldMessage)) {
        deleteMessage($URL, $chat_id, $oldMessage);//удаляем старое сообщение
    }
    /*
        if (isset($oldMessage)) {
        $res1 = $mysqli->query("SELECT * FROM Language WHERE ID_COUNTRY='" . $lang . "' AND ID_PHRASE=12");
        while ($row1 = $res1->fetch_assoc()) {
        editMessageText($URL, $chat_id, $oldMessage, $row1['PHRASE']);//заменяем старое сообщение на нужную фразу
        }
        }
    */
...
function deleteMessage($URL, $chat_id, $message_id)
{
...
function editMessageText($URL, $chat_id, $message_id, $message, $keyboard)
{
    $message = text($message);//заменяем символы с \\
    $keyboard = text($keyboard);//заменяем символы с \\
    if (isset($keyboard)) {
...
function newsletter($URL, $mysqli, $ID, $TS, $str, $postLink, $statCount, $language, $VK_ACCESS_TOKEN)
{
    if ($TS == 'VK') {
    ...
    if ($language == 'ENG') {
        sendMessage($URL, $mysqli,  '@TheVenusProject', $str, false);
    } else {
        if ($language == 'RUS') {
            sendMessage($URL, $mysqli,  '@TheVenusProjectRUS', $str, false);
        }
    }

    $res = $mysqli->query("SELECT * FROM `Subscription` WHERE SOURCE_ID = " . $ID);
    while ($row = $res->fetch_assoc()) {
        sendMessage($URL, $mysqli,  $row['USER_ID'], $str, false);
    }
...
function handler($VK_ACCESS_TOKEN, $post_id, $str) //Обрабатывает посты в вк. Если В посте только видео и нет текста, заменяет ссылку на пост на ссылку на видео.
{
    ...
    if ($update['response'][0]['text'] == "") {
        if (isset($update['response'][0]['attachments'])) {
            ...
            } elseif (count($update['response'][0]['attachments']) == 1 && $update['response'][0]['attachments'][0]['type'] == "link" &&
                isset($update['response'][0]['attachments'][0]['link']['button'])) {
                $str = substr_replace($str,
                    str_replace("m.vk.com", 'vk.com', $update['response'][0]['attachments'][0]['link']['url']),
                    strpos($str, "https://vk.com/"),
                    strpos($str, "\">") - strpos($str, "https://vk.com/"));
            }
        }
    }
    return $str;
...
if (strlen($content) <= 1) {//если скрипт вызван без аргументов, то проверяем ресурсы на наличие новых постов
    ...
    while ($row = $res->fetch_assoc()) {
        if ($row['ENABLED'] == 1) {
            if ($row['TYPE_SOURCE'] == "FB") {
                $posts = array();
                $content1 = file_get_contents("https://graph.facebook.com/" . $row['ARG1'] . "/feed?fields=id&limit=10&access_token=369530936733221|07-7I0g6jb0fSvTLn-iUsAvJsmc");
                $update1 = json_decode($content1, TRUE);
                    ...
                    if ($update1["data"][0]["id"] != $row['LAST_POST']) {
                        ...
                        if (count($posts) > 0) {//если хоть один пост для публикации есть
                            ...
                            for ($b = count($posts) - 1; $b >= 0; $b--) {
                                ...
                                //newsletter($URL, $mysqli, $row['ID'], $row['TYPE_SOURCE'], $name . " \r\n" . "https://fb.com/" . $posts[$b], "https://fb.com/" . $posts[$b], $postCount, $row['ID_COUNTRY'], $VK_ACCESS_TOKEN);//посылаем всем, кто подписан на $row['ID'] сообщение с названием ресурса и ссылкой на пост
                                newsletter($URL, $mysqli, $row['ID'], $row['TYPE_SOURCE'], '<a href="https://fb.com/' . $posts[$b] . '">' . $name . '</a>', "https://fb.com/" . $posts[$b], $postCount, $row['ID_COUNTRY'], $VK_ACCESS_TOKEN);//посылаем всем, кто подписан на $row['ID'] сообщение с названием ресурса и ссылкой на пост
                            }
                ...
                $posts = array();
                $content1 = file_get_contents("https://www.googleapis.com/youtube/v3/search?key=AIzaSyDps5AMIn6hexgxJhfum8Kfbyn2OauCMy4&channelId=" . $row['ARG1'] . "&part=id&order=date&maxResults=10");
                $update1 = json_decode($content1, TRUE);
                    ...
                    if ($update1["items"][0]["id"]["videoId"] != $row['LAST_POST']) {
                        ...
                        if (count($posts) > 0) {
                            ...
                            for ($b = count($posts) - 1; $b >= 0; $b--) {
                                if ($posts[$b] != "") {
                                    $mysqli->query("INSERT INTO `Posts_History`(`SOURCE_ID`, `POST_ID`, `VIEWS`) VALUES  (" . $row['ID'] . ", '" . $posts[$b] . "', " . $postCount . ")");
                                    //newsletter($URL, $mysqli, $row['ID'], $row['TYPE_SOURCE'], $name . " \r\n" . "https://www.youtube.com/watch?v=" . $posts[$b], "https://www.youtube.com/watch?v=" . $posts[$b], $postCount, $row['ID_COUNTRY'], $VK_ACCESS_TOKEN);
                                    newsletter($URL, $mysqli, $row['ID'], $row['TYPE_SOURCE'], '<a href="https://www.youtube.com/watch?v=' . $posts[$b] . '">' . $name . '</a>', "https://www.youtube.com/watch?v=" . $posts[$b], $postCount, $row['ID_COUNTRY'], $VK_ACCESS_TOKEN);
                                }
                            }
                ...
                if ($update1["response"]["items"][0]["id"] != $row['LAST_POST']) {
                    ...
                    if (count($posts) > 0) {
                        ...
                        for ($b = count($posts) - 1; $b >= 0; $b--) {
                            if ($posts[$b] != "0") {
                                $mysqli->query("INSERT INTO `Posts_History`(`SOURCE_ID`, `POST_ID`, `VIEWS`) VALUES  (" . $row['ID'] . ", '" . $posts[$b] . "', " . $postCount . ")");
                                //newsletter($URL, $mysqli, $row['ID'], $row['TYPE_SOURCE'], $name . " \r\n" . $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b], $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b], $postCount, $row['ID_COUNTRY'], $VK_ACCESS_TOKEN);
                                newsletter($URL, $mysqli, $row['ID'], $row['TYPE_SOURCE'], '<a href="' . $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b] . '">' . $name . '</a>', $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b], $postCount, $row['ID_COUNTRY'], $VK_ACCESS_TOKEN);
                            }
                        }
                ...
                if ($update1["items"][0]["code"] != $row['LAST_POST']) {
                    ...
                    if (count($posts) > 0) {
                        ...
                        for ($b = count($posts) - 1; $b >= 0; $b--) {
                            $mysqli->query("INSERT INTO `Posts_History`(`SOURCE_ID`, `POST_ID`, `VIEWS`) VALUES  (" . $row['ID'] . ", '" . $posts[$b] . "', " . $postCount . ")");
                            newsletter($URL, $mysqli, $row['ID'], $row['TYPE_SOURCE'], '<a href="https://www.instagram.com/p/' . $posts[$b] . '/">' . $name . '</a>', 'https://www.instagram.com/p/' . $posts[$b] . '/', $postCount, $row['ID_COUNTRY'], $VK_ACCESS_TOKEN);
                            //newsletter($URL, $mysqli, $row['ID'], $row['TYPE_SOURCE'], $name . " \r\n" . 'https://www.instagram.com/p/' . $posts[$b] . '/', 'https://www.instagram.com/p/' . $posts[$b] . '/', $postCount, $row['ID_COUNTRY'], $VK_ACCESS_TOKEN);
                        }
    ...
    if ($presence == false) {//если в БД пользователя с таким chat_id нет
        ...
        $update1 = sendMessage($URL, $mysqli,  $chatID, "Choose language:", false, json_encode($resp));
        ...
        if ($typeMessage == 2 || ($typeMessage == 1 && $callbackData[0] == 3)) {//если сообщение от пользователя или от кнопки "Назад"
            ...
            if ($typeMessage == 2) {//если сообщение от пользователя
                ...
                $update1 = editMessageText($URL, $chatID, $re['ID_MESSAGE'], $message, json_encode($resp));
            }
            ...
            if ($callbackData[0] == 10) { //Выход
                deleteMessage($URL, $chatID, $re['ID_MESSAGE'], $row1['PHRASE']);//удаляем старое сообщение
            } else {
                $update1 = editMessageText($URL, $chatID, $re['ID_MESSAGE'], $message, json_encode($resp));
            }
        }
