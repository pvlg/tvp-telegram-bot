...
const URL = "https://api.telegram.org/bot319975635:AAEMUd4tFZjZvu0mZPCNWGbKmu7cAasO47s";
...
if (strlen($content) <= 1) {//если скрипт вызван без аргументов, то проверяем ресурсы на наличие новых постов
    ...
    while ($row = $res->fetch_assoc()) {
        if ($row['ENABLED'] == 1) {
            if ($row['TYPE_SOURCE'] == "FB") {
                ...
                if (isset($update1["data"])) {//если существует массив с постами
                    ...
                    if (count($posts) > 0) {//если хоть один пост для публикации есть
                        ...
                        for ($b = count($posts) - 1; $b >= 0; $b--) {
                            ...
                            //newsletter($row['ID'], $row['TYPE_SOURCE'], $name . " \r\n" . "https://fb.com/" . $posts[$b], "https://fb.com/" . $posts[$b], $postCount, $row['ID_COUNTRY']);//посылаем всем, кто подписан на $row['ID'] сообщение с названием ресурса и ссылкой на пост
                            newsletter($row['ID'], $row['TYPE_SOURCE'], '<a href="https://fb.com/' . $posts[$b] . '">' . $name . '</a>', "https://fb.com/" . $posts[$b], $postCount, $row['ID_COUNTRY']);//посылаем всем, кто подписан на $row['ID'] сообщение с названием ресурса и ссылкой на пост
                        }
                    ...
                    if (count($posts) > 0) {
                        ...
                        for ($b = count($posts) - 1; $b >= 0; $b--) {
                            if ($posts[$b] != "") {
                                ...
                                newsletter($row['ID'], $row['TYPE_SOURCE'], '<a href="https://www.youtube.com/watch?v=' . $posts[$b] . '">' . $name . '</a>', "https://www.youtube.com/watch?v=" . $posts[$b], $postCount, $row['ID_COUNTRY']);
                            }
                        }
                ...
                $posts = array();
                $content1 = file_get_contents("https://api.vk.com/method/wall.get?owner_id=-" . $row['ARG1'] . "&count=10&v=5.60&access_token=eb72b0b6eb72b0b6eb72b0b6cfeb2e57d8eeb72eb72b0b6b23c5b75d5081ce705e04377");
                $update1 = json_decode($content1, TRUE);
                ...
                if (count($posts) > 0) {
                    ...
                    $name = $row['NAME_SOURCE'];
                    for ($b = count($posts) - 1; $b >= 0; $b--) {
                        if ($posts[$b] != "0") {//HOT FIX
                            $mysqli->query("INSERT INTO `Posts_History`(`SOURCE_ID`, `POST_ID`, `VIEWS`) VALUES  (" . $row['ID'] . ", '" . $posts[$b] . "', " . $postCount . ")");
                            //newsletter($row['ID'], $row['TYPE_SOURCE'], $name . " \r\n" . $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b], $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b], $postCount, $row['ID_COUNTRY']);
                            newsletter($row['ID'], $row['TYPE_SOURCE'], '<a href="' . $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b] . '">' . $name . '</a>', $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b], $postCount, $row['ID_COUNTRY']);
                        }
                    }
                ...
                if (count($posts) > 0) {
                    ...
                    for ($b = count($posts) - 1; $b >= 0; $b--) {
                        $mysqli->query("INSERT INTO `Posts_History`(`SOURCE_ID`, `POST_ID`, `VIEWS`) VALUES  (" . $row['ID'] . ", '" . $posts[$b] . "', " . $postCount . ")");
                        newsletter($row['ID'], $row['TYPE_SOURCE'], '<a href="https://www.instagram.com/p/' . $posts[$b] . '/">' . $name . '</a>', 'https://www.instagram.com/p/' . $posts[$b] . '/', $postCount, $row['ID_COUNTRY']);
    ...
    if (isset($update['callback_query'])) {//если ответ сообщение пришло от кнопки
        ...
        //if($chatID == 164316126) customNewsletter();
    }
...
function newsletter($ID, $TS, $str, $postLink, $statCount, $language)
{
    if ($language == 'ENG') {
