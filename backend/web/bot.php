<?php
			...
			if ($row['ENABLED'] == 1) {
					...
					if (isset($update1["data"])) {//если существует массив с постами
						...
						if($r == true) {
							...
							for ($b = 0; $b < count($posts); $b++) {
								...
								while ($row1 = $res1->fetch_assoc()) $existence = $row1['POST_ID'];
								if($existence != 0) { 
									$mysqli->query("UPDATE `Source` SET `LAST_POST`='" . $existence . "' WHERE `ID`=" . $row['ID']);
									break;
								}
								else $posts1[$b] = $posts[$b];
							}
							$posts = array();
							$posts = $posts1;
							if(count($posts) != count($posts1)) $r = false;
						}
						if (count($posts) > 0) {//если хоть один пост для публикации есть 
							...
							for ($b = count($posts) - 1; $b >= 0; $b--) {
								...
								newsletter($row['ID'], $name . " \r\n" . "https://fb.com/" . $posts[$b], $postCount);//посылаем всем, кто подписан на $row['ID'] сообщение с названием ресурса и ссылкой на пост
							}
				...
				elseif ($row['TYPE_SOURCE'] == "YT") {//аналогично FB
					...
					if ($update1['pageInfo']['totalResults'] > 0) {
						...
						if (count($posts) > 0) {
							...
							for ($b = count($posts) - 1; $b >= 0; $b--) {
								$mysqli->query("INSERT INTO `Posts_History`(`SOURCE_ID`, `POST_ID`, `VIEWS`) VALUES  (".$row['ID'].", '".$posts[$b]."', ".$postCount.")");
								newsletter($row['ID'], $name . " \r\n" . "https://www.youtube.com/watch?v=" . $posts[$b], $postCount);
							}
					...
					if (count($posts) > 0) {
						...
						for ($b = count($posts) - 1; $b >= 0; $b--) {
							if($posts[$b] != "0"){//HOT FIX
								$mysqli->query("INSERT INTO `Posts_History`(`SOURCE_ID`, `POST_ID`, `VIEWS`) VALUES  (".$row['ID'].", '".$posts[$b]."', ".$postCount.")");
								newsletter($row['ID'], $name . " \r\n" . $row['LINK_SOURCE'] . "?w=wall-" . $row['ARG1'] . "_" . $posts[$b], $postCount);
							}
						}
				...
				elseif ($row['TYPE_SOURCE'] == "IG") {//аналогично FB
					...
					if (count($posts) > 0) {
						...
						for ($b = count($posts) - 1; $b >= 0; $b--) {
							$mysqli->query("INSERT INTO `Posts_History`(`SOURCE_ID`, `POST_ID`, `VIEWS`) VALUES  (".$row['ID'].", '".$posts[$b]."', ".$postCount.")");
							newsletter($row['ID'], $name . " \r\n" . 'https://www.instagram.com/p/' . $posts[$b] . '/', $postCount);
						}
	...
	'" . json_encode($update) . "'," . time() . ",'" . date(DATE_RFC822) . "')");*/
	...
	function newsletter($ID, $str, $statCount)
	{
		global $mysqli;
